...
void printResult(HUSKYLENSResult result);     // Function declaration for printing HuskyLens result
void moveServos(int xPos, int yPos);          // Function declaration for moving servos
...
void loop() {
    ...
    if (currentTime - previousCheckTime >= checkInterval) {   // Check if it's time to request data from HuskyLens
        ...
        if (!huskylens.request()) {              // Request data from HuskyLens
            ...
            Serial.println(F("###########"));
            while (huskylens.available()) {      // Process the detected object(s)
...
void moveServos(int xPos, int yPos) {
    ...
    delay(15);                                  // Delay to allow the servos to reach the desired position
}
